    ...
    return (
      <div className="p-6 text-center">
        ...
        <button
          ...
          className="mt-4 px-4 py-2 rounded-lg border border-[var(--border-color)]"
        >
          {t("assets.back")}
        </button>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-4">
            <button
              onClick={() => navigate("/dashboard/assets")}
              className="p-2 rounded-lg border border-[var(--border-color)] hover:bg-[var(--bg-secondary)]"
            >
              <ArrowLeft size={20} />
            </button>
            <div>
              ...
              <p className="text-[var(--color-secondary)]">{asset.assetType}</p>
            </div>
          </div>
          <button
        ...
        {/* Tabs */}
        <div className="bg-[var(--surface-color)] rounded-2xl shadow-sm border border-[var(--border-color)] mb-6">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-4 border-b-2 transition-colors whitespace-nowrap ${
                    activeTab === tab.id
                      ...
                      : "border-transparent text-[var(--color-secondary)] hover:text-[var(--text-color)]"
                  }`}
                >
                  <Icon size={20} />
                  {tab.label}
          ...
          <div className="p-6">
                    ...
                    {asset.maintenanceHistory.map((maintenance, index) => (
                      <div key={index} className="border border-[var(--border-color)] rounded-lg p-4">
                        ...
                        {maintenance.description && (
                          <p className="text-sm text-[var(--color-secondary)] mt-2">{maintenance.description}</p>
                        )}
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-center text-[var(--color-secondary)] py-8">{t("assets.maintenance.no_history")}</p>
                )}
              </div>
            )}
            ...
            {activeTab === "insurance" && (
              <div className="space-y-6">
                ...
                {asset.insurancePolicies && asset.insurancePolicies.length > 0 ? (
                      ...
                      <div key={index} className="border border-[var(--border-color)] rounded-lg p-4">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            ...
                            <p className="text-sm text-[var(--color-secondary)]">מספר פוליסה: {policy.policyNumber}</p>
                          </div>
                          <span
                            className={`px-2 py-1 rounded-full text-xs ${
                              policy.isActive ? "bg-green-100 text-green-700" : "bg-[var(--bg-secondary)] text-[var(--text-color)]"
                            }`}
                          >
                  ...
                  <p className="text-center text-[var(--color-secondary)] py-8">{t("assets.insurance.no_policies")}</p>
                )}
              </div>
            )}
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.depreciation.purchase_price")}</p>
                    <p className="text-2xl font-bold">
                      {asset.purchasePrice?.toLocaleString() || 0} {asset.purchaseCurrency || "ILS"}
                    </p>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.depreciation.current_value")}</p>
                    <p className="text-2xl font-bold">
                  ...
                  <div className="bg-red-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.depreciation.accumulated")}</p>
                    <p className="text-2xl font-bold">
                      ...
                      <table className="w-full border-collapse">
                        <thead>
                          <tr className="bg-[var(--bg-secondary)]">
                            <th className="border border-[var(--border-color)] px-4 py-2 text-right">תקופה</th>
                            <th className="border border-[var(--border-color)] px-4 py-2 text-right">סכום פחת</th>
                            <th className="border border-[var(--border-color)] px-4 py-2 text-right">פחת מצטבר</th>
                            <th className="border border-[var(--border-color)] px-4 py-2 text-right">ערך נוכחי</th>
                          </tr>
                        </thead>
                        <tbody>
                          {asset.depreciationHistory.map((entry, index) => (
                            <tr key={index}>
                              <td className="border border-[var(--border-color)] px-4 py-2">{entry.period}</td>
            ...
            {activeTab === "performance" && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.performance.utilization_rate")}</p>
                    <p className="text-3xl font-bold">
                      {asset.performance?.utilizationRate?.toFixed(1) || 0}%
                    </p>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.performance.efficiency")}</p>
                    <p className="text-3xl font-bold">{asset.performance?.efficiency?.toFixed(1) || 0}%</p>
                  </div>
                  <div className="bg-yellow-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.performance.downtime")}</p>
                    <p className="text-3xl font-bold">{asset.performance?.downtime || 0}</p>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.performance.mtbf")}</p>
            ...
            {activeTab === "costs" && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.costs.purchase_cost")}</p>
                    <p className="text-2xl font-bold">
                  ...
                  <div className="bg-yellow-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.costs.maintenance_cost")}</p>
                    <p className="text-2xl font-bold">
                  ...
                  <div className="bg-green-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.costs.operating_cost")}</p>
                    <p className="text-2xl font-bold">
                  ...
                  <div className="bg-purple-50 p-4 rounded-lg">
                    <p className="text-sm text-[var(--color-secondary)] mb-1">{t("assets.costs.tco")}</p>
                    <p className="text-2xl font-bold">
